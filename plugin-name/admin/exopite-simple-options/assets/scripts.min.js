...
if (typeof throttle !== "function") {
    ...
    Plugin.prototype = {
        init: function () { this.bindEvents(); if (this.$container.data('sortable')) { this.$container.sortable({ axis: "y", cursor: "move", handle: '.exopite-sof-accordion__title', tolerance: "pointer", distance: 5, opacity: 0.5, }) } }, bindEvents: function () {
            var plugin = this; plugin.$container.off().on('click' + '.' + plugin._name, '.exopite-sof-accordion__title', function (e) { e.preventDefault(); plugin.toggleAccordion.call(plugin, $(this)) }); plugin.$container.on('sortstart' + '.' + plugin._name, function () { plugin.$element.trigger('exopite-sof-accordion-sortstart', [plugin.$container]) }); plugin.$container.on('sortstop' + '.' + plugin._name, function () {
                if (plugin.$container.parents('.exopite-sof-wrapper-metabox').length && plugin.$container.data('sortable')) { plugin.$container.find('.exopite-sof-accordion__item').each(function (index_item) { $(this).find('[name^="' + plugin.$container.data('name') + '"]').each(function () { var $this_name = $(this).attr('name'); var $name_prefix = plugin.$container.data('name'); $name_prefix = $name_prefix.replace(/\[/g, '\\[').replace(/]/g, '\\]'); var regex = new RegExp($name_prefix + '\\[\\d+\\]'); var $this_name_updated = $this_name.replace(regex, plugin.$container.data('name') + '\[' + index_item + '\]'); $(this).attr('name', $this_name_updated) }) }) }
                plugin.$element.trigger('exopite-sof-accordion-sortstop', [plugin.$container]); plugin.isSortableCalled = !0
            })
        }, unbindEvents: function () { this.$container.off('.' + this._name) }, toggleAccordion: function ($header) {
